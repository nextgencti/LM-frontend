...
import { 
  ...
  RotateCcw, Save, FileCheck, AlertCircle, Calendar, Phone, Users, Loader, ChevronLeft, ChevronDown, TestTube
} from 'lucide-react';
...
const BookingForm = ({ 
  ...
  const handlePaidAmountChange = (val) => {
    let paidVal = parseFloat(val);
    if (isNaN(paidVal)) paidVal = '';
    const paid = Math.min(paidVal || 0, newBooking.totalAmount);
  ...
  const handleDiscountChange = (val) => {
    let discVal = parseFloat(val);
    if (isNaN(discVal)) discVal = '';
                       ...
                       {filteredPatients.map(p => (
                         ...
                         >
                            <div>
                              ...
                              <p className="text-[10px] font-medium text-slate-400 uppercase mt-0.5">#{p.patientId || p.id?.substring(0, 8)}</p>
                            </div>
              ...
              <div className="flex gap-2">
                <div className="flex-grow relative group">
                  <input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-[13px] font-bold text-brand-dark outline-none focus:ring-4 focus:ring-brand-primary/10 focus:border-brand-primary transition-all placeholder:text-slate-400 placeholder:font-medium shadow-sm"
                    value={testSearch}
                  ...
                  />
                  <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400 group-focus-within:text-brand-primary transition-colors cursor-pointer pointer-events-none" />
                  {(isTestSearchFocused || testSearch) && filteredTests.length > 0 && (
                    <div className="absolute top-full left-0 w-full mt-2 bg-white border border-slate-200 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] z-50 overflow-y-auto max-h-[300px] divide-y divide-slate-100 p-1 custom-scrollbar">
                       {filteredTests.map(t => (
                         <button 
                          key={t.id}
                          onClick={() => {
                          ...
                          className="w-full text-left px-3 py-2.5 rounded-lg hover:bg-slate-50 flex items-center justify-between group transition-colors"
                         >
                           <div>
                             <p className="text-[13px] font-bold text-brand-dark leading-tight capitalize group-hover:text-brand-primary transition-colors">{t.testName}</p>
                             <p className="text-[10px] font-medium text-slate-500 uppercase mt-0.5 tracking-wider">{t.methodology || '---'}</p>
                           </div>
                           <div className="text-right">
                             <p className="text-[13px] font-bold text-brand-dark tabular-nums tracking-tight">₹{t.price}</p>
                             <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Price</p>
                           </div>
                         </button>
                       ))}
                    </div>
          ...
          <div className="bg-white rounded-[40px] shadow-[0_32px_128px_rgba(0,0,0,0.3)] max-w-2xl w-full border border-white/20 animate-in zoom-in-95 duration-500 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSaveQuickPatient} className="flex-grow flex flex-col overflow-hidden">
              <div className="p-6 sm:p-8 space-y-6 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="relative w-[130px] shrink-0">
                      <select 
                        className={`w-full pl-3 pr-8 py-2.5 border rounded-xl transition-all font-bold text-[13px] outline-none cursor-pointer appearance-none shadow-sm ${
                          quickPatient.isAuto ? 'bg-blue-50/50 border-blue-200 text-blue-600 focus:bg-white focus:border-brand-primary/50' : 'bg-slate-50 border-slate-200 text-brand-dark focus:border-brand-primary/50 focus:bg-white'
                        }`}
                      ...
                      <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none">
                        <ChevronDown className={`w-4 h-4 ${quickPatient.isAuto ? 'text-blue-400' : 'text-slate-400'}`} />
                      </div>
                    </div>
                    <input 
                      required 
                      ...
                      onChange={e => {
                        ...
                        setQuickPatient({...quickPatient, name: formattedName});
                      }} 
                    />
                  </div>
                </div>
                ...
                <div className="space-y-0.5">
                  <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider ml-1">Address</label>
                  <textarea 
                    rows="2"
            ...
            <form onSubmit={handleSaveQuickDoctor} className="flex-grow flex flex-col overflow-hidden">
              <div className="p-6 sm:p-8 space-y-6 overflow-y-auto custom-scrollbar">
                ...
                <div className="space-y-0.5">
                  ...
                  <div className="flex gap-2">
                    ...
                    <input 
                      ...
                      onChange={e => {
                        const formattedName = e.target.value.split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ');
                        setQuickDoctor({...quickDoctor, name: formattedName});
                      }} 
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="space-y-0.5">
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider ml-1">Email <span className="text-brand-dark lowercase tracking-normal font-bold opacity-80">(Needed for reports)</span></label>
                    <input 
                      type="email" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div className="space-y-0.5">
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider ml-1">Commission Structure *</label>
                    <div className="flex gap-2">
                  ...
                  <div className="space-y-0.5">
                    <label className="text-[10px] font-semibold text-slate-400 uppercase tracking-wider ml-1">Account Status</label>
                    <div className="relative w-full">
                      <select 
                        className="w-full pl-3 pr-8 py-2.5 bg-slate-50 border border-slate-200 focus:bg-white focus:border-brand-primary/50 rounded-xl transition-all font-bold text-[13px] text-brand-dark outline-none cursor-pointer appearance-none shadow-sm"
